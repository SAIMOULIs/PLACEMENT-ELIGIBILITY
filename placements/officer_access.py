...
def request_access(request):
    ...
    if request.method == 'POST':
        ...
        password = request.POST.get('password', ''); confirm = request.POST.get('confirm_password', '')
        full_name = request.POST.get('full_name', '').strip(); institution = request.POST.get('institution', '').strip()
        designation = request.POST.get('designation', '').strip(); phone = request.POST.get('phone', '').strip(); reason = request.POST.get('reason', '').strip()
        if not all([username, email, password, full_name, institution, designation]): error = 'Please complete all required fields.'
        elif len(username) < 3: error = 'Username must contain at least 3 characters.'
        elif len(password) < 8: error = 'Password must contain at least 8 characters.'
        elif password != confirm: error = 'Passwords do not match.'
        elif User.objects.filter(username__iexact=username).exists(): error = 'That username is already in use.'
...
def approve_request(request, pk):
    ...
    if request.method == 'POST' and access_request.status == 'pending':
        access_request.user.role = 'officer'; access_request.user.is_active = True
        access_request.user.save(update_fields=['role', 'is_active'])
        access_request.status = 'approved'; access_request.reviewed_at = timezone.now(); access_request.reviewed_by = request.user; access_request.review_notes = 'Approved by administrator.'
...
def reject_request(request, pk):
    access_request = get_object_or_404(OfficerAccessRequest, pk=pk)
    if request.method == 'POST' and access_request.status == 'pending':
        access_request.user.is_active = False; access_request.user.save(update_fields=['is_active'])
        access_request.status = 'rejected'; access_request.reviewed_at = timezone.now(); access_request.reviewed_by = request.user; access_request.review_notes = 'Rejected by administrator.'
        access_request.save(update_fields=['status', 'reviewed_at', 'reviewed_by', 'review_notes'])
    return redirect('officer_requests')
